    ...
    return (
        <main className="p-10">
                        ...
                        <div className="flex flex-row gap-4 items-center justify-center mb-4">
                        {
                            ...
                            return (
                                <div key={option.value} className="flex gap-1">
                                    ...
                                    />
                                    <label htmlFor={option.value} className="h-10 flex items-center mr-3 cursor-pointer">{option.label}</label>
                                </div>
                            )
                            })
                        }
            ...
            <div className="w-full relative">
                <p className="flex gap-1 mb-4 items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                    ...
                    <span>선택된 영상 : {selectRadio ? selectRadio : '없음'}</span>
                </p>
                <p className="mb-4 pl-7">
                    선택된 영상 src : { videoSrc ? videoSrc : '없음' }
                </p>
                <div className="w-full h-full rounded-2xl overflow-hidden border">
                    {
                        isVideoView ? <video ref={videoRef} className="w-full h-full object-cover" src={`./video/${selectRadio}.mp4`} controls></video> : <div className="py-[150px] flex justify-center items-center">선택 된 영상이 없습니다.</div>
                    }
                </div>
            </div>
        </main>
