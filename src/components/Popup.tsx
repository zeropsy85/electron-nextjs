...
export default function Popup({ isLayerView, popupInfo, handleLayerClose }: PopupProps) {
    ...
    useEffect(()=>{
        ...
        isLayerView ? videoRef.current?.play() : videoRef.current?.pause();
    },[isLayerView]);
    ...
    return (
        <div ref={layerRef} className="fixed top-0 left-0 h-full w-full flex justify-center items-center bg-black bg-opacity-70 z-10 opacity-0 invisible" onClick={handleLayerClose}>
            <div className="border rounded-lg bg-white relative p-10 flex flex-col justify-center" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="mt-6">
                    {
                        popupInfo.media_type === 'VIDEO' ? <video ref={videoRef} className="w-[500px] " src={popupInfo.media_url} controls /> : <PrintTemplate imgSrc={popupInfo.media_url} />
                    }
                </div>
                {
                    popupInfo.media_type !== 'VIDEO' && 
